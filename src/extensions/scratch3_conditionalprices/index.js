 ...
 * @constructor
 */
class Scratch3ConditionalPrices {
    constructor (runtime) {
        /**
    ...
    _getMusicState (target) {
        let musicState = target.getCustomState(Scratch3ConditionalPrices.STATE_KEY);
        if (!musicState) {
            musicState = Clone.simple(Scratch3ConditionalPrices.DEFAULT_MUSIC_STATE);
            target.setCustomState(Scratch3ConditionalPrices.STATE_KEY, musicState);
        }
        return musicState;
    }
    ...
    _playDrumNum (util, drumNum) {
        ...
        if (this._concurrencyCounter > Scratch3ConditionalPrices.CONCURRENCY_LIMIT) {
            return;
        }
    ...
    playNoteForBeats (args, util) {
        if (this._stackTimerNeedsInit(util)) {
            ...
            note = MathUtil.clamp(note,
                Scratch3ConditionalPrices.MIDI_NOTE_RANGE.min, Scratch3ConditionalPrices.MIDI_NOTE_RANGE.max);
            let beats = Cast.toNumber(args.BEATS);
            beats = this._clampBeats(beats);
     ...
     */
    _clampBeats (beats) {
        return MathUtil.clamp(beats, Scratch3ConditionalPrices.BEAT_RANGE.min, Scratch3ConditionalPrices.BEAT_RANGE.max);
    }
     ...
     */
    _updateTempo (tempo) {
        tempo = MathUtil.clamp(tempo, Scratch3ConditionalPrices.TEMPO_RANGE.min, Scratch3ConditionalPrices.TEMPO_RANGE.max);
...
module.exports = Scratch3ConditionalPrices;
